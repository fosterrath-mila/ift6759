...
def prepare_dataloader(
    ...
    Returns:
        A ``torch.utils.data.DataLoader`` object that can be used to produce input tensors for your model. One tensor
    ...
    class DummyDataset(Dataset):
        def __init__(self, target_datetimes):
            """
        ...
        def __len__(self):
            return len(self.target_datetimes)

        def __getitem__(self, index):

            samples = np.random.uniform(
                size=(self.image_dim[0], self.image_dim[1], self.n_channels)
            )
            targets = np.zeros(shape=(self.output_seq_len,))
            return (
                torch.tensor(samples, dtype=torch.float32),
                torch.tensor(targets, dtype=torch.float32),
            )

    dataset = DummyDataset(target_datetimes)
...
def prepare_model(
    ...
    Returns:
        A ``torch.nn.Module`` object that can be used to generate new GHI predictions given imagery tensors.
    """
    ...
    class DummyModel(torch.nn.Module):
        ...
        def __init__(self, number_of_target_time_offsets):
            super(DummyModel, self).__init__()

            flat_dimension = self.n_channels * self.image_dim[0] * self.image_dim[1]
            output_dimension = number_of_target_time_offsets

            self.dense1 = torch.nn.Linear(flat_dimension, self.hidden_layer_dimension)
            self.activation1 = torch.nn.ReLU()
            self.dense2 = torch.nn.Linear(self.hidden_layer_dimension, output_dimension)
            self.activation2 = torch.nn.Softmax()

        def forward(self, inputs: torch.tensor):
            flat_inputs = inputs.flatten(start_dim=1)

            hidden_units = self.activation1(self.dense1(flat_inputs))
            outputs = self.activation2(self.dense2(hidden_units))
            return outputs

    model = DummyModel(len(target_time_offsets))
...
def generate_predictions(
    data_loader: DataLoader, model: torch.nn.Module, pred_count: int
) -> np.ndarray:
    ...
    with tqdm.tqdm("generating predictions", total=pred_count) as pbar:
        for iter_idx, minibatch in enumerate(data_loader):
            assert (
                isinstance(minibatch, list) and len(minibatch) >= 2
